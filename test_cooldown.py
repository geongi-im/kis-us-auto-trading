...
def test_cooldown_logic():
    ...
    # TradingBot 인스턴스 생성
    bot = TradingBot()
    print(f"설정된 쿨다운 시간: {bot.cooldown_minutes}분")
    ...
    # 마지막 매수 주문 시간 조회
    last_buy_time_str = bot.get_last_buy_order_time()
    ...
    if last_buy_time_str:
        ...
        # 쿨다운 체크
        if time_diff < bot.cooldown_minutes:
            remaining_minutes = bot.cooldown_minutes - time_diff
            print(f"[X] 매수 쿨다운 중: {remaining_minutes:.1f}분 후 가능")
            return False
        else:
            print(f"[O] 쿨다운 완료: 매수 가능")
            return True
    else:
...
def test_should_buy_logic():
    """should_buy 함수의 쿨다운 로직 테스트"""
    print("\n=== should_buy 함수 쿨다운 로직 테스트 ===")
    ...
    try:
        if not bot.strategy.load_historical_data():
            print("⚠️ RSI 데이터 로드 실패 - 쿨다운 로직만 테스트")
            # RSI 체크를 건너뛰고 직접 쿨다운 로직 테스트
            ...
            if last_buy_time_str:
                ...
                print(f"시간 차이: {time_diff:.1f}분")
                print(f"쿨다운 설정: {bot.cooldown_minutes}분")
                
                if time_diff < bot.cooldown_minutes:
                    remaining_minutes = bot.cooldown_minutes - time_diff
                    print(f"[X] 쿨다운 중: {remaining_minutes:.1f}분 후 가능")
                else:
                    print(f"[O] 쿨다운 완료")
            else:
                print("[O] 매수 주문 내역 없음")
            ...
            # 실제 should_buy 함수 호출
            can_buy = bot.should_buy(test_price)
            print(f"should_buy 결과: {can_buy}")
...
def test_time_calculation():
    ...
    for diff, desc in tests:
        if diff < cooldown:
            ...
            print(f"{desc}: [X] 쿨다운 중 ({remaining:.1f}분 후 가능)")
        else:
            print(f"{desc}: [O] 쿨다운 완료")

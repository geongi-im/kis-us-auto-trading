...
def parseTradingTickers():
    """TRADING_TICKERS 환경변수를 파싱하여 딕셔너리로 반환"""
    trading_tickers_str = os.getenv("TRADING_TICKERS", "")
    if not trading_tickers_str:
        raise Exception("TRADING_TICKERS 환경변수가 설정되지 않았습니다.")
    ...
    try:
        ...
        if not trading_tickers:
            raise Exception("TRADING_TICKERS에서 유효한 ticker를 찾을 수 없습니다.")
...
def checkEnvVariables():
    ...
    required_vars = ['APP_KEY', 'APP_SECRET', 'ACCOUNT_NO', 'IS_VIRTUAL', 
                     ...
                     'COOLDOWN_MINUTES', 'CHECK_INTERVAL_MINUTES', 'TRADING_TICKERS']
